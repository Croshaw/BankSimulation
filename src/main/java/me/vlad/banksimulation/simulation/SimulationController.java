...
public class SimulationController {
    ...
    public SimulationController(int seed, Duration durationOfSimulation, long secondsStep, double dailyClerkSalary, ValueRange<Long> requestDistributionInterval, ValueRange<Long> durationOfRequestServicing, ValueRange<Double> profitOfRequestServicing) {
        this.seed = seed;
    ...
    private void tryGenerateRequest() {
        ...
        while(Duration.between(lastGeneratedRequest, bank.getCurrentDateTime()).toSeconds() >= range && !bank.isQueueFull()) {
            System.out.println("Cur between: %d\t Range: %d".formatted(Duration.between(lastGeneratedRequest, bank.getCurrentDateTime()).toSeconds(), range));
            getRandomClient(random).comeToBank(bank, Duration.ofSeconds(durationOfRequestServicing.getRandomValueFromRange(random)), profitOfRequestServicing.getRandomValueFromRange(random));
